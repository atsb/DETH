...
void DisplayAbout()
{
	...
	BCINT y0 = (ScrMaxY - height) / 2;
	HideMousePointer();
    DrawScreenWindow( x0, y0, width, height, " Help: About", YELLOW);
    DrawScreenButtonIn( x0 + 3, y0 + 21, width - 6, height - 24, "", FALSE);
    SetColor(BLACK);
#ifdef GAME_HEXEN
    ...
    SetColor(BLUE);
    DrawScreenText( -1, -1, " ");
    DrawScreenText( -1, -1, "Maintained By:");
    SetColor(BLACK);
    DrawScreenText( -1, -1, "Adam 'Gibbon' Bilbrough");
    DrawScreenText( -1, -1, " ");
    SetColor(BLUE);
    DrawScreenText( -1, -1, "Special Thanks:");
    SetColor(BLACK);
    ...
    SetColor( YELLOW);
    DrawScreenText( -1, y0 + height - 20, "Press any key to close...");
    bioskey(0);
	ShowMousePointer();
}
...
void DisplayRIP()
{
    ...
    BCINT y0 = (ScrMaxY - height) / 2;
    HideMousePointer();
    DrawScreenWindow( x0, y0, width, height, " Legends: R.I.P", YELLOW);
    DrawScreenButtonIn( x0 + 3, y0 + 21, width - 6, height - 24, "", FALSE);
    SetColor(BLACK);
#ifdef GAME_HEXEN
    DrawScreen3DText( x0 + 14, y0 + 34, BLACK, TRUE, "HETH - Hexen Editor for Total Headcases");
    ...
    DrawScreenText( x0 + 14, y0 + 44, " ");
    SetColor(BLACK);
    DrawScreenText( -1, -1, "Since the last release of DETH");
    DrawScreenText( -1, -1, "some important members have since");
    DrawScreenText( -1, -1, "passed.");
    DrawScreenText( -1, -1, " ");
    DrawScreenText( -1, -1, "John W. Anderson.");
    DrawScreenText( -1, -1, "Jim F. Flynn.");
    DrawScreenText( -1, -1, "Ty Halderman.");
    DrawScreenText( -1, -1, " ");
    DrawScreenText( -1, -1, "We salute you awesome Doomers!");
    DrawScreenText( -1, -1, "R.I.P");
    SetColor( YELLOW);
    DrawScreenText( -1, y0 + height - 20, "Press any key to close...");
    bioskey(0);
    ShowMousePointer();
