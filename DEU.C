...
void Credits( FILE *where)
{
  ...
  fprintf( where, "Authors:-\n");
    ...
    fprintf( where, "    Adam 'Gibbon' Bilbrough\n");
...
void ParseInput(char *input)
{
  ...
  int x, wadcount;
	...
	else if (!strcmp( com, "?"))
	{
        ...
        DrawScreenText(  -1, -1, "[BSP] <WadFile>                -  Execute BSP.EXE against chosen Patch WAD");
#ifdef GAME_HEXEN
  ...
  for (;;)
	{
		...
		else if (!strcmp( com, "?"))
		{
            ...
            printf( "[BSP] <WadFile>                   -- to execute BSP.EXE against chosen patch wad file\n");
			printf( "!                                 -- drop to dos\n");
